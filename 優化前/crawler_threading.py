...
def process_review(r_buffer): #  msg = [title, score, price, date, tag]
    global global_csv_data
    for r_buf in r_buffer:
        reviews_link = r_buf[1]
        msg = r_buf[0]
        ...
        for d in data:
            global_csv_data.append(d)

def crawler(start_page, processes):
    global reviews_buffer
    ...
    while page < page_range:
        ...
        for product in products_list:
            ...
            for reviews_page in range(1, int(reviews_pages) + 1):
                process_review([msg, reviews_link_template + str(reviews_page)])
...
for i in range(1, processes + 1):
    tasks.append(threading.Thread(target=crawler, args=(i, processes),))
    tasks[i - 1].start()
